        ...
        initCalendar: function() {
            if (jQuery().fullCalendar) {
                var e = new Date,
                    ...
                    i = e.getFullYear(),
                    l = {};
                $("#calendar_").width() <= 400 ? ($("#calendar_").addClass("mobile"), l = {
                    left: "title, prev, next",
                    center: "",
                    right: "today,month,agendaWeek,agendaDay"
                }) : ($("#calendar_").removeClass("mobile"), l = App.isRTL() ? {
                    right: "title",
                    center: "",
                ...
                // $("#calendar_").fullCalendar("destroy"), $("#calendar_").fullCalendar({
                // $('#calendar_').fullCalendar({
                	// eventClick: function(event) {
			          // alert("summary in-progress...");
			        // },
                // })
                // $(document).ready(function() {
                	// $.post(baseUrl +"cashier/get_calendar_events",function(data){
					    ...
					    // if(data==""){
                            ...
                            $(document).on("click",".fc-past",function(){
                              ...
                              swal({
                                        title: '', 
                                        html: true,
                                        text: 'Can not add reservation less than your current date. Please select another date.',  
    				        ...
    				        $(document).on("click",".fc-today, .fc-future",function(){
    				          var date_selected = $(this).attr('data-date');
    						...
    						var newDate =convertDate(date_selected);
    				          $("#date-selected").val(newDate);
    				          // $("#pickTimeModal").modal('show');
    				        });
					        $(document).on("click",".fc-today, .fc-future",function(){
                                // $('#calendar_').fullCalendar('changeView', 'agendaDay');
                                // $('#calendar_').fullCalendar({
                                //     customButtons: {
                                //         myCustomButton: {
                                //                             text: 'custom!',
                        ...
                        $(document).on("click",".fc-more",function(){
				      	...
				      	$('#calendar_').fullCalendar({
	                        ...
	                        eventSources: [
	                            ...
	                            // your event source
	                            {
	                              url: baseUrl + 'custs_bank/get_reservation_date',
	                              method: 'POST',
	                              extraParams: {
	                          ...
	                          eventClick: function(info) {
                                    if(info.stats == 'void'){
                                        var jbtn = 'swal-btn-hide';                                   
                                    }else{
                                        var jbtn = 'btn-primary'; 
                                    }
                                    // $(document).ready(function() {
                                    //     $("button.confirm").addClass('haha');
                                    // });
	                                // var details = 'ORDER# ' + info.sales_id+'Time: ' + info.title+'<br />' + 'Name: ' + info.name;
	                                var details = 
	                                			  '<span style="float:left;font-weight:700;color:#000;">Reservation No: '+ info.trans_ref+
                                                  '</span> <br><span style="float:left;color:#000;">Reference No: '+ info.ref_no+
                                                  '</span> <br> <span style="float:left;color:#000;">Transaction Date: '+ info.tdate+
                                                  '</span> <br> <span style="float:left;color:#000;">Reservation Date: '+ info.sdate+
                                                  '</span> <br> <span style="float:left;color:#000;">Table/Room No: '+ info.table_name+
                                                  '</span> <br> <span style="float:left;color:#000;">Deposited Amount: '+ info.total_amount+
                                                  '</span> <br> ';
	                                var dets = info.sales_id;

	                                swal({
	                                    title: '<span style="float:left;font-weight:700;color:#000;">'+ info.name+'</span> <span style="float:right;font-weight:700;color:#000;"> Status: '+ info.status+'</span><br>', 
	                                    html: true,
	                                    text: details,  
	                                    // confirmButtonText: "V redu", 
	                                    allowOutsideClick: "true" ,
                                        ...
                                        confirmButtonClass: jbtn,
                                        confirmButtonText: `Edit/Recall`,
                                        denyButtonText: `Cancel`,
	                                }, function (result) {
                                      // Your code
                                      if (result) {
                                        var date_selected2 = info.istart;
                                        // window.location = baseUrl+'cashier/update_set_schedule';
                                        function convertDate(inputFormat) {
                                          function pad(s) { return (s < 10) ? '0' + s : s; }
                                          var d = new Date(inputFormat)
                                          return [pad(d.getMonth()+1), pad(d.getDate()), d.getFullYear()].join('/')
                                        }   
                                        var newDate =convertDate(date_selected2);
                                        $("#date-selected2").val(newDate);
                                        $("#time-select2").timepicker({
                                            showInputs: false
                                        });
                                        // for select dropdown table id
                                        var o = new Option(info.table_name, info.table_id);
                                        $(o).html(info.table_name);
                                        $("select#tbl_id").append(o);
                                        $(o).attr('selected', 'selected');
                                        $(o).attr('hidden', 'hidden');

                                        $("#staticBackdropLabel2").text(info.name);
                                        // $("#tbl_id").val(info.table_name);
                                        $( "#date-selected2" ).datepicker({format:'mm/dd/yyyy'});
                                        $("#pickTimeModal2").modal('show');
                                        var date_selected = $("#date-selected2").val();
                                        var time_selected = $("#time-select2").val();
                                        var tsales_id = $("input[type=hidden]#tsales_id").val(info.sales_id);
                                        var ttype = $("input[type=hidden]#ttype").val(info.ttype);
                                        var status = $("input[type=hidden]#status").val(info.stats);
                                        var ref_no = $("input[type=hidden]#ref_no").val(info.ref_no);
                                        // alert(info.sales_id);
                                        // return false;
                                        // if(info.sales_id != null){
                                            // $('.cancel-pop').addClass('swal-btn-hide');
                                            // $('.void-pop').removeClass('swal-btn-hide');
                                        // }else{
                                            // $('.void-pop').addClass('swal-btn-hide');
                                            // $('.cancel-pop').removeClass('swal-btn-hide');
                                        // }
                                        $(document).on("click","#set-schedule-submit2",function(){
                                            // alert(info.sales_id);
                                            // return false;
                                          var date_selected = $("#date-selected2").val();
                                          var time_selected = $("#time-select2").val();
                                          ...
                                          $.post(baseUrl + "cashier/update_set_date",{'date_selected':date_selected ,'time_selected':time_selected,'sales_id':info.sales_id,'cust_id':info.cust_id,'cust_amount':info.cust_amount,'add_deposit':add_deposit,'ref_no':info.ref_no,'tbl_id':tbl_id},function(data){
                                            // alert(data);
                                            // return false;
                                            var d = JSON.parse(data);
                                            var msgs = parseFloat(d.msg);
                                            if(msgs == 1){
                                                alert('Please select another time.');
                                                return false;
                                            }
                                            else if(msgs == 2){
                                                alert('Cannot update reservation less than current date.');
                                                return false;
                                            }
                                            else{
                                                ...
                                                swal({
                                                      position: 'center',
                                                      icon: 'success',
                                                      title: 'Change date successfully!',
                        ...
                        $(".fc-right").append('<span class="fa fa-times fa-2x" id="fc-right-close"></span>');
					    $(document).find("[data-date='2021-01-01']").removeClass("fc-future");
					    $(document).find("[data-date='2021-01-01']").addClass("fc-past");
				    // },"json");
				    // });
                     $(document).on("click","#fc-right-close",function(){
                        window.location.href = "../cashier";
                     });
				    $(document).on("click","#set-schedule-submit",function(){
				    	// alert('haha');
				    	// return false;
				      var date_selected = $("#date-selected").val();
				      var time_selected = $("#time-select").val();
				      // var antigen = $("#antigen_val").val();
				      // if(time_selected == ""){
				      //   toastr.error('You must select time.',{timeOut: 800000});
				      //   return false;
				      // }
				      $.post(baseUrl + "cashier/set_schedule_data",{'date_selected':date_selected ,'time_selected':time_selected},function(data){
				      	// alert(data);
				      	var t = JSON.parse(data);
        				var msg = parseFloat(t.msg);
				      	if(msg == 1){
				        // toastr.error('You must select time.',{timeOut: 800000});
				        	alert('Please select another time.');
				        	return false;
				      		...
				      		// alert('HAHA');
				        	// return false;
				      		window.location.href = "../custs_bank/reservation_cust_bank";
				      	}
				      });
				    });

				    // $(document).find(".fc-day").addClass("newyear");
				        // $(document).find("fc-day").on("click",function(){
				        //   alert(2);
				        // });
				  // });
            }
