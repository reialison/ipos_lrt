		...
		function showTransType(data,ctr){	
			...
			$.each(data,function(key,val){
				...
				// var sCol = $('<div class="col-md-5" style="border-right-style:groove"></div>');
	 			...
	 			$('<button/>')
	 			.attr({'id':val['text_id']+'-btn','ref':val['text_id'],'class':'new-order-btns btn-block cpanel-btn-blue '+val['new_trans_type'],'style':'height:60px;width:130px;margin-top:1px;'})
	 			.text(val['text'])
											...
											// if(default_count < total_mod){
												...
												$.each(opt.details,function(mod_id,det){
													...
													// if(det.default != 1){
														...
														$('<button/>')
														.attr({'id':'mod-'+mod_id,'ref':mod_id,'class':'counter-btn-silver btn btn-block btn-default mods-btns','style':'color:'+opt.branch_text_color+'!important;background-color:#43B0F0!important;height:60px;width:130px'})
									...
									if(!$.isEmptyObject(submod)){
										...
										$.each(submod,function(mod_sub_id,opt){
												...
												// $.each(opt.details,function(mod_id,det){
													...
													$('<button/>')
													.attr({'id':'submod-'+mod_sub_id,'ref':mod_sub_id,'class':'counter-btn-silver btn btn-block btn-default submods-btns','style':'color:'+opt.branch_text_color+'!important;background-color:#43B0F0!important;height:60px;width:130px'})
													// .css({'margin':'5px','width':'130px'})
													.text(opt.name)
													.appendTo(sCol)
													.click(function(){
				...
				$.post(baseUrl+'cashier/get_menus_search_sorted',formData,function(data){
					...
					var div = $('.menus-div .items-lists').append('<div class="row"></div>');
			 		$.each(data,function(key,opt){
			 			...
			 			.attr({'id':'menu-'+menu_id,'ref':menu_id,'qty':opt.qty,'class':'counter-btn-silver btn btn-block btn-default','style':'height:60px;width:130px;color:'+opt.branch_text_color+'!important;background-color:#43B0F0!important'})
		...
		function showMenuCategories(data,ctr){
		 ...
		 // 	},'json');
			...
			$.each(data,function(key,val){
				...
				// 	}
				// }
				var sCol = $('<div class="col-md-3 media-margin-category-negative"></div>');


	 			$('<button/>')
	 			.attr({'id':'menu-cat-'+cat_id,'ref':cat_id,'class':'btn-block category-btns counter-btn-blue double btn btn-default','style':'height:60px;width:130px;margin-bottom:1px;color:'+val['branch_text_color']+'!important;background-color:'+val['bcolor']+'!important'})
	 			.text(val.name)
	 			// .appendTo('.menu-cat-container')
	 			.appendTo(sCol)
	 			.click(function(){
					...
					// data.show_img = 1;
			 		if(data.show_img == 1){
						...
						$.each(data.jsons,function(key,opt){
				 			...
				 			$('<button/>')
				 			.attr({'id':'menusub-'+menu_sub_id,'ref':menu_sub_id,'class':'counter-btn-green btn btn-block btn-default','style':'height:60px;width:130px;color:'+opt.branch_text_color+'!important;background-color:#43B0F0!important'})
				 			.text(opt.name)
				 			.appendTo(sCol)
				 			.click(function(){
				 				var btn = $(this);
				 		...
				 		$.each(data.json,function(key,opt){
				 			...
				 			var sCol = $('<div class="col-md-3 media-margin-menu"></div>');
				 			$('<button/>')
				 			.attr({'id':'menu-'+menu_id,'ref':menu_id,'qty':opt.qty,'class':'counter-btn-silver btn btn-block btn-default menu-btns','style':'height:60px;width:130px;color:'+opt.branch_text_color+'!important;background-color:#43B0F0!important'})
				 			.prop('disabled',unavailable)
				 			// .text(opt.name)
				 			.html(opt.name + unavailable_text)
				 			.appendTo(sCol)
			 	...
			 	$.post(baseUrl+'cashier/get_menus_subcat_sorted/'+id,formData,function(data){
					...
					if(data.show_img == 1){
						...
						$.each(data.json,function(key,opt){
			 				...
			 				$('<button/>')
			 				.attr({'id':'menu-'+menu_id,'ref':menu_id,'class':'counter-btn-silver btn btn-block btn-default','style':'height:60px;width:130px;margin-bottom:1px;color:'+opt.branch_text_color+'!important;background-color:#43B0F0!important'})
			 				.text(opt.name)
				 			.appendTo(sCol)
				 			.click(function(){
				 				var btn = $(this);
	...
	<?php elseif($use_js == 'settleJs'): ?>
		...
		function showPaymentType(data,ctr){	
			...
			$.each(data,function(key,val){
				...
				var sCol = $('<div class="nav-div nav-div col-md-2 col-sm-2 col-lg-2  text-left "></div>');
				// var sCol = $('<div class="col-md-5" style="border-right-style:groove"></div>');
