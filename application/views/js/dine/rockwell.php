...
$(document).ready(function(){
	<?php if($use_js == 'mainPageJs'): ?>
		$('#content').rLoad({url:'rockwell/daily_files'});
		$('#daily-files-btn').click(function(){
			$('#content').rLoad({url:'rockwell/daily_files'});
			return false;
		});
		$('#settings-btn').click(function(){
			$('#content').rLoad({url:'rockwell/settings'});
			return false;
		});
	<?php elseif($use_js == 'dailyFilesJS'): ?>
		$('#show-file-btn').click(function(){
			...
			var btn = $(this);
			btn.goLoad2();
			$.post(baseUrl+'rockwell/get_file','date='+date,function(data){
		...
		$('#regen-btn').click(function(){
			...
			var btn = $(this);
			btn.goLoad2();
			$.post(baseUrl+'rockwell/regen_file','date='+date,function(data){
				// alert(data);
				// console.log(data);
				if(data.error == 0){
					rMsg(data.msg,'success');
					$.post(baseUrl+'rockwell/get_file','date='+date,function(data2){
