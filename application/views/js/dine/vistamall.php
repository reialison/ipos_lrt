...
$(document).ready(function(){
	<?php if($use_js == 'mainPageJs'): ?>
		$('#content').rLoad({url:'vistamall/daily_files'});
		$('#daily-files-btn').click(function(){
			$('#content').rLoad({url:'vistamall/daily_files'});
			return false;
		});
		$('#settings-btn').click(function(){
			$('#content').rLoad({url:'vistamall/settings'});
			return false;
		});
	<?php elseif($use_js == 'dailyFilesJS'): ?>
		$('#show-file-btn').click(function(){
			...
			var btn = $(this);
			btn.goLoad2();
			$.post(baseUrl+'vistamall/get_file','date='+date,function(data){
		...
		$('#regen-btn').click(function(){
			...
			var btn = $(this);
			btn.goLoad2();
			$.post(baseUrl+'vistamall/regen_file','date='+date,function(data){
				if(data.error == 0){
					rMsg(data.msg,'success');
					$.post(baseUrl+'vistamall/get_file','date='+date,function(data2){
