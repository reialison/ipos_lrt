...
class Rockwell extends Reads {
	public function __construct(){
		parent::__construct();
		$this->load->helper('dine/rockwell_helper');
		$this->load->model('dine/cashier_model');
		$this->load->model('core/trans_model');
	}
	public function index(){
        $data = $this->syter->spawn(null);
        $data['code'] = rockwellPage();
        ...
        $data['load_js'] = 'dine/rockwell.php';
	...
	public function settings(){
		$data = $this->syter->spawn(null);
		$objs = $this->site_model->get_tbl('rockwell');
		$obj = array();
		if(count($objs) > 0){
			$obj = $objs[0];			
		}
	...
	public function settings_db(){
		...
		$id = $this->input->post('rockwell_id');
		$this->site_model->update_tbl('rockwell','id',$items,$id);
		$this->main_model->update_tbl('rockwell','id',$items,$id);
		$msg = "Updated Settings.";
		echo json_encode(array('id'=>$id,'msg'=>$msg));
	}
	public function get_file(){
		$date = $this->input->post('date');
		$mall_res = $this->site_model->get_tbl('rockwell');
		$daily_str = "";
		if(count($mall_res) > 0){
	...
	public function regen_file(){
		...
		if($zread != 0){
			$reg = $this->rockwell_file($zread,true);
