...
class WindowsPrintConnector implements PrintConnector
{
     ...
     */
    protected function runWrite($data, $filename,$x=0)
    {
        try{
            // echo $filename;die();
            $filterable = $check_net_use = shell_exec("net use 2>&1"); // checks the status of the LPT printers @jx01282020
            $expr_raw = explode("\t", $check_net_use);

            if(isset($expr_raw[0])){ // we need to format the raw return of shell_exec to make if able to filter status @jx01282020
            ...
            if(stripos($filterable, "OK ".$filename) !== false ){ // check if printer is OK  @jx01282020
              // throw new Exception('TEST');exit;
               $check_print = file_put_contents($filename, $data);
               return $check_print; 
            }else{
                // if($x < 5){ // try thre times
                //     $x++;
                  
                //     $this->runWrite($data . $x,$filename,$x);
                //     throw new Exception("Printer is not available. Please make sure printer is connected correctly.".$x);exit;
                // }else{
                    // throw new Exception('TEST 3');
                    ping_printer_connection();
                    // bgExec("php -q ".printer_cron_path);
                    // system("cmd /c start ".restart_printer.""); 
                // }
            }
            // var_dump($filename);die();
        } catch (Exception $e) {
       
               throw new Exception("Printer is not available. Please make sure printer is connected correctly. 1 ".$x ." ".$e);exit;
