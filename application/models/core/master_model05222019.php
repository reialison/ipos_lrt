		...
		// die();
			...
			if(empty($migration_id)){
				...
				if($automated){
					...
					$trans_id_raw = $this->main_db->select('sales_id')->where('master_id' ,NULL)->get($table_name)->result();
					$json_encode  = json_encode($trans_id_raw);
					
				}				
			}

			// $this->compose_raw_logs($trans_id_raw);
...
// migrate add trans sales_discounts  @Justin
	...
	public function update_trans_sales_charges($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		 $table_name = 'trans_sales_charges';
			...
			if($automated){
				...
				$trans_raw = $this->main_db->select('trans_sales_charges.*')->join($table_name,'trans_sales.sales_id = trans_sales_charges.sales_id','left')->where('(trans_sales.update_date > "'.$update_date.'" OR trans_sales.update_date IS NULL)',NULL, false)->where('trans_sales_charges.sales_id is not null',null,false)->get('trans_sales')->result();
	...
	public function update_trans_sales_menus($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		...
		// die();
			...
			if($automated){
				...
				$trans_raw = $this->main_db->select('trans_sales_menus.*')->join($table_name,'trans_sales.sales_id = trans_sales_menus.sales_id','left')->where('(trans_sales.update_date > "'.$update_date.'" OR trans_sales.update_date IS NULL)',NULL, false)->where('trans_sales_menus.sales_id is not null',null,false)->get('trans_sales')->result();			
	...
	public function update_trans_sales_discounts($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		 $table_name = 'trans_sales_discounts';
			...
			if($automated){
				...
				$trans_raw = $this->main_db->select('trans_sales_discounts.*')->join($table_name,'trans_sales.sales_id = trans_sales_discounts.sales_id','left')->where('(trans_sales.update_date > "'.$update_date.'" OR trans_sales.update_date IS NULL)',NULL, false)->where('trans_sales_discounts.sales_id is not null',null,false)->get('trans_sales')->result();
	...
	public function update_trans_sales_items($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		 $table_name = 'trans_sales_items';
			...
			if($automated){
				...
				$trans_raw = $this->main_db->select('trans_sales_items.*')->join($table_name,'trans_sales.sales_id = trans_sales_items.sales_id','left')->where('(trans_sales.update_date > "'.$update_date.'" OR trans_sales.update_date IS NULL)',NULL, false)->where('trans_sales_items.sales_id is not null',null,false)->get('trans_sales')->result();
	...
	public function update_trans_sales_loyalty_points($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		 $table_name = 'trans_sales_loyalty_points';
			...
			if($automated){
				...
				// $trans_id_raw = $this->main_db->select($selected_field)->where('`datetime` > ',$update_date)->get($table_name)->result();
				// $trans_based_id_raw = $this->main_db->select($based_delete_id)->where('`datetime` > ',$update_date)->group_by($based_delete_id)->get($table_name)->result();
				$trans_raw = $this->main_db->select('trans_sales_loyalty_points.*')->join($table_name,'trans_sales.sales_id = trans_sales_loyalty_points.sales_id','left')->where('(trans_sales.update_date > "'.$update_date.'" OR trans_sales.update_date IS NULL)',NULL, false)->where('trans_sales_loyalty_points.sales_id is not null',null,false)->get('trans_sales')->result();
	...
	public function update_trans_sales_menu_modifiers($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		 $table_name = 'trans_sales_menu_modifiers';
			...
			if($automated){
				...
				// $trans_id_raw = $this->main_db->select($selected_field)->where('`datetime` > ',$update_date)->get($table_name)->result();
				// $trans_based_id_raw = $this->main_db->select($based_delete_id)->where('`datetime` > ',$update_date)->group_by($based_delete_id)->get($table_name)->result();
				$trans_raw = $this->main_db->select('trans_sales_menu_modifiers.*')->join($table_name,'trans_sales.sales_id = trans_sales_menu_modifiers.sales_id','left')->where('(trans_sales.update_date > "'.$update_date.'" OR trans_sales.update_date IS NULL)',NULL, false)->where('trans_sales_menu_modifiers.sales_id is not null',null,false)->get('trans_sales')->result();
	...
	public function update_trans_sales_no_tax($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		 $table_name = 'trans_sales_no_tax';
			...
			if($automated){
				...
				$trans_raw = $this->main_db->select('trans_sales_no_tax.*')->join($table_name,'trans_sales.sales_id = trans_sales_no_tax.sales_id','left')->where('(trans_sales.update_date > "'.$update_date.'" OR trans_sales.update_date IS NULL)',NULL, false)->where('trans_sales_no_tax.sales_id is not null',null,false)->get('trans_sales')->result();
	...
	public function update_trans_sales_payments($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		 $table_name = 'trans_sales_payments';
			...
			if($automated){
				// $trans_raw = $this->main_db->select('trans_sales_payments.*')->where('master_id', $migrate_local_id)->get($table_name)->result();
				// $trans_id_raw = $this->main_db->select($selected_field)->where('master_id', $migrate_local_id)->get($table_name)->result();
				// $trans_based_id_raw = $this->main_db->select($based_delete_id)->where('master_id', $migrate_local_id)->group_by($based_delete_id)->get($table_name)->result();
				...
				// $trans_id_raw = $this->main_db->select($selected_field)->where('`datetime` > ',$update_date)->get($table_name)->result();
				// $trans_based_id_raw = $this->main_db->select($based_delete_id)->where('`datetime` > ',$update_date)->group_by($based_delete_id)->get($table_name)->result();
				$trans_raw = $this->main_db->select('trans_sales_payments.*')->join($table_name,'trans_sales.sales_id = trans_sales_payments.sales_id','left')->where('(trans_sales.update_date > "'.$update_date.'" OR trans_sales.update_date IS NULL)',NULL, false)->where('trans_sales_payments.sales_id is not null',null,false)->get('trans_sales')->result();
	...
	public function update_trans_sales_tax($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		 $table_name = 'trans_sales_tax';
			...
			if($automated){
				...
				// $trans_id_raw = $this->main_db->select($selected_field)->where('`datetime` > ',$update_date)->get($table_name)->result();
				// $trans_based_id_raw = $this->main_db->select($based_delete_id)->where('`datetime` > ',$update_date)->group_by($based_delete_id)->get($table_name)->result();
				$trans_raw = $this->main_db->select('trans_sales_tax.*')->join($table_name,'trans_sales.sales_id = trans_sales_tax.sales_id','left')->where('(trans_sales.update_date > "'.$update_date.'" OR trans_sales.update_date IS NULL)',NULL, false)->where('trans_sales_tax.sales_id is not null',null,false)->get('trans_sales')->result();
	...
	public function update_trans_sales_zero_rated($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		 $table_name = 'trans_sales_zero_rated';
			...
			if($automated){
				...
				$trans_raw = $this->main_db->select('trans_sales_zero_rated.*')->join($table_name,'trans_sales.sales_id = trans_sales_zero_rated.sales_id','left')->where('(trans_sales.update_date > "'.$update_date.'" OR trans_sales.update_date IS NULL)',NULL, false)->where('trans_sales_zero_rated.sales_id is not null',null,false)->get('trans_sales')->result();
...
// migrate update trans_voids  -  @Justin
	...
	public function download_menus($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		    ...
		    $selected_field = 'menu_id,menu_code,menu_barcode,menu_name,menu_short_desc,
		    					menu_cat_id,menu_sub_cat_id,menu_sched_id,cost,reg_date, update_date,no_tax,miaa_cat,
...
// echo "asd";die();
	...
	public function download_modifiers($migration_id=NULL, $update_date = NULL, $automated=false, $migrate_local_id=NULL){
		    $table_name = 'modifiers';
		    $selected_field = 'mod_id, name, cost, has_recipe, reg_date, inactive';
	...
	public function execute_migration(){
		...
		$finish_log =  $this->get_finish_log();
			...
			// // add records from main to master
			 ...
			 if(empty($finish_log)){
				...
				$add_functions_array = array('add_trans_sales','add_trans_sales_local_tax',
					...
					'add_trans_refs'	,'add_users','add_receipt_discounts','add_tables','add_coupons','migrate_branch_details',
					'add_trans_receiving_menu','add_trans_receiving_menu_details','add_trans_receivings','add_trans_receiving_details','add_menu_moves','add_item_moves','add_reasons','add_locations','add_suppliers','add_terminals');
			 }else{
			...
			$update_functions_array = array('update_trans_sales','update_trans_sales_charges','update_trans_sales_discounts',
											...
											'update_menu_moves','update_item_moves','update_locations','update_suppliers','update_terminals');
