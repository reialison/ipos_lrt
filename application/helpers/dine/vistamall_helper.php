...
function settingsPage($obj=array()){
	$CI =& get_instance();
		$CI->make->sForm('vistamall/settings_db',array('id'=>'settings-form'));
			$CI->make->hidden('vistamall_id',iSetObj($obj,'id'));
			$CI->make->sDiv(array('style'=>'padding:10px;'));
				$CI->make->sDivRow();
					$CI->make->sDivCol(4,'left',4);
						$CI->make->H(3,'STALL CODE',array('class'=>'headline text-center','style'=>'margin-bottom:10px;font-size:20px;'));
						$CI->make->input(null,'stall_code',iSetObj($obj,'stall_code'),'',array('class'=>'count-inputs big-man-input rOkay','ro-msg'=>'tenant code must not be empty'));
